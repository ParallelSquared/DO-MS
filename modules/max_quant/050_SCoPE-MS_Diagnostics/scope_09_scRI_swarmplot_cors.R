init <- function() {
  ...
  type <- 'plot'
  box_title <- 'Spearman correlation of carrier channel to all other channels'
  help_text <- 'Plotting spearman correlation of carrier channel to all other channels. Carrier channel determined as channel with max median reporter ion intensity. '
  ...
  .plotdata <- function(data, input) {
    ...
    #plotdata<-melt(plotdata[,c(raw.ind, ri.ind)], id.vars = c("Raw.file"))
    #plotdata<-plotdata[,c(raw.ind, ri.ind)]
    
    #plotdata$value[plotdata$value==Inf]<-NA

    # plotdata[,ri.ind]<-log2(plotdata[,ri.ind])
    # plotdata[,ri.ind][(plotdata[,ri.ind])==Inf]<-NA
    # plotdata[,ri.ind][(plotdata[,ri.ind])==-Inf]<-NA
    
    plotdata<-plotdata %>% 
      group_by(Raw.file) %>%
      do(data.frame(Cor=t(cor(.[,ri.ind], .[,ri.max], method="spearman"))))    
  ...
  .plot <- function(data, input) {
    ...
    ggplot(plotdata, aes(x=Raw.file, y=value, color=variable)) +
